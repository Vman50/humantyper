...
class ExternalTypingSimulatorApp:
    ...
    def estimate_remaining_seconds(self, text: str, idx: int) -> float:
        ...
        if self.enable_thinking.get():
            ...
            # sentence pauses: count sentence terminators roughly in the remaining text
                sentence_count = sum(1 for i, ch in enumerate(remaining) if ch in '.!?')
                thinking_overhead += sentence_count * float(self.sentence_pause_seconds.get())
        ...
        try:
            ...
            for i, char in enumerate(text):
                ...
                if self.enable_thinking.get():
                    ...
                    if char in '.!?':
                        time.sleep(self.sentence_pause_seconds.get())
